...
angular.module('angularParseInterface', [
  ...
  .factory('parseInterface', function ($rootScope, $resource, $log) {
    ...
    var parseInterface = {};
//    parseInterface.$rootScope = $rootScope;

    parseInterface._resource = (function () {

      // resource submodule
      var resourceMod = {};

      // The url submodule. This provides helpers for putting together the url.
      resourceMod._url = (function () {
        var restApiBaseUrl = 'https://api.parse.com/1';
        var prefixUrl = function (url, prefix) {
          return prefix.replace(/\/+$/, '') + '/' + url.replace(/^\/+/, '');
        };

        return {
          prependBaseUrl: function (url) {
            return prefixUrl(url, restApiBaseUrl);
          }
        };
      }());

      resourceMod._headers = (function () {

        // This returns appropriate request headers
        var parseRequestHeadersGetter = function (appConfig, appStorage) {
          var headers = {
        ...
        // This wraps the transformRequest function in another function that first adds the parse request headers and
        // then calls the original function
        var wrapTransformRequest = function (transformRequest, appConfig, appStorage) {
          return function (data, headersGetter) {
            angular.extend(headersGetter(), parseRequestHeadersGetter(appConfig, appStorage));
            return transformRequest(data, headersGetter);
          };
        };

        // This adds or modifies the transformRequest function for a given action so that the action will add Parse
        // request headers to every request
        var wrapTransformReqForAction = function (action, wrapperFx/*, appConfig, appStorage*/) {
          var wrapperArgs = [].slice.call(arguments, 2);
          var origTransformRequest = action.transformRequest || function (data/*, headersGetter*/) { return angular.toJson(data); };
          wrapperArgs.unshift(origTransformRequest);
          action.transformRequest = wrapperFx.apply(null, wrapperArgs);
//          action.transformRequest = wrapTransformRequest(origTransformRequest, appConfig, appStorage);
        };

        return {
          _parseRequestHeadersGetter: parseRequestHeadersGetter,
          _wrapTransformRequest: wrapTransformRequest,
          _addHeadersForAction: wrapTransformReqForAction,
          ...
          addParseRequestHeaders: function (actions, appConfig, appStorage) {
            angular.forEach(actions, function (action) {
              wrapTransformReqForAction(action, wrapTransformRequest, appConfig, appStorage);
            });
          }
        };
      }());
      ...
      resourceMod._dataEncoding = (function () {
        ...
        var typeOfRequestData = function (Resource, fieldName, val) {
        ...
        var typeOfResponseData = function (Resource, fieldName, val) {
          var dataType = Resource._getFieldDataType(fieldName) || (val && val.__type) || typeof val;
          return dataType[0].toUpperCase() + dataType.slice(1);
        };

//        var classOfResponseData = function (Resource, fieldName, val) {
//          return val && val.className;
//        };

        var isBlacklisted = function (Resource, fieldName) {
...
//              Resource._setFieldDataType(fieldName, 'Date');
      ...
      resourceMod.createAppResourceFactory = function (baseResourceFactory, appConfig, appStorage, appEventBus) {
...
//          console.log('signing out');
    ...
    // query submodule
    parseInterface._query = (function () {
      var queryMod = {};

      var Query = queryMod.Query = function (Resource) {
...
//          objectDecorator = this.objectDecorator;
    ...
    parseInterface._user = (function () {

      // user submodule
      return {
        _userUrl: '/:urlRoot/:objectId',
        _userDefaults: {urlRoot: 'users', objectId: '@objectId'},
        _userDecorator: function (Resource, appStorage, appEventBus) {
        ...
        createUserModel: function (appResourceFactory, appStorage, appEventBus) {
    ...
    parseInterface.createAppInterface = function (appConfig, clientStorage) {
      ...
      appInterface.User = this._user.createUserModel(appInterface._appResourceFactory, appStorage, appEventBus);
